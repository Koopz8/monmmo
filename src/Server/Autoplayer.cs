...
/// <param name="Gives">A creature it hands over, if it hands one over.</param>
...
public static class Autoplayer
{
        ...
        for (int pass = 1; pass <= MostPasses; pass++)
        {
            ...
            foreach (MapData map in world.Maps.Where(m => reach.Maps.Contains(m.Id)))
            {
                foreach (uint address in Reachable(map, stood, flags))
                {
                    ...
                    {
                        if (moves.Add(move)) opened = true;
                    }

                    // Whatever it hands over. The first of these is the starter, and without
                    // it nothing after it can be fought at all.
                    if (did.Gives is { } species && party.Count < 6
                        && battles.Wild(species, StartingLevel) is { } given)
                    {
                    ...
                    switch (Fight(battles, rules, party, trainerId))
                    {
                        case true:
                            won++;
                            break;
    ...
    private static bool? Fight(
        BattleFactory battles, GameRules rules, List<SavedMon> party, int trainerId)
    {
        ...
        // Whatever survived, written back — including the damage. A run that healed between
        // every fight would be measuring a different game.
